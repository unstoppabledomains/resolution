...
class SizeChecker {
  ...
  async fetchSize(packageName: string, packageVersion: string) {
    ...
    this.log(
      `${response.name} @ ${response.version} --> ${this.toKB(
        response.size,
      )} KB`,
      Color.FgBlue,
    );
  ...
  async main() {
    ...
    const color: Color =
      this.sizeLimit >= this.totalSize ? Color.FgGreen : Color.FgRed;
    this.log(`Total Size: ${this.toKB(this.totalSize)} KB`, color);
    if (color == Color.FgRed) return this.fail();
    return this.success();
  }
  ...
  private fail() {
    this.log(
      `Size limit was exceeded ${this.toKB(this.totalSize)} KB >= ${this.toKB(
        this.sizeLimit,
      )} KB`,
      Color.FgRed,
    );
