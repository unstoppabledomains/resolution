...
type ResolverRecordsStructure = { crypto?: { [key: string]: { address: string } }; ttl?: string, [key: string]: any };

type ZNSResolve = {
	crypto?: { [key: string]: { address: string } }; ttl?: string, [key: string]: any
}
...
export default class {
	...
	async getResolverRecordsStructure(resolverAddress: string):
		...
		const resolverRecords =
			((await this.getContractField(resolver, 'records')) as ResolverRecord[]) || [];
		const result = {};
	...
	async resolve(domain: string | undefined): Promise<ZNSResolve | null> {
		...
		const registryRecord = registryRecords.find(
			(r: { key: string | null }) => r.key == nodeHash,
		);
