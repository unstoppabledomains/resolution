...
export default class {
	...
	async getResolverRecordsStructure(resolverAddress: string) {
		...
		const resolverRecords =
			((await this.getContractField(resolver, 'records')) as Array<{
				key: _.Many<string | number | symbol>;
				val: any;
			}>) || [];
		const result = {};
	...
	async resolve(domain: string | undefined) {
		...
		const registryRecord = registryRecords.find((r: { key: string | null; }) => r.key == nodeHash);
		...
		const resolution = (await this.getResolverRecordsStructure(
			resolverAddress,
		)) as { crypto?: Array<{ address?: any }>; ttl?: string | undefined };
