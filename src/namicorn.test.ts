...
it('reverses address to ENS domain', async () => {
	...
	const eye = jest.spyOn(ens, '_resolverCallToName').mockImplementation(() => 'adrian.argent.xyz');
	const secondEye = jest.spyOn(ens, '_getResolver').mockImplementation(() => '0xDa1756Bb923Af5d1a05E277CB1E54f1D0A127890');
...
it('reverses address to ENS domain null', async () => {
	...
	// mockAPICalls('ENS', testName, MAINNET_URL);
	const ens = new Ens(MAINNET_URL);
	const spy = jest.spyOn(ens, '_getResolver').mockImplementation(() => '0x0000000000000000000000000000000000000000');
	const result = await ens.reverse(
...
it('resolves .xyz name using ENS blockchain', async () => {
	...
	const spy = jest.spyOn(namicorn.ens, '_getResolutionInfo').mockImplementation(() =>
		new Promise((resolve, reject) => resolve(
			['0xb0E7a465D255aE83eb7F8a50504F3867B945164C', Number(0x00), '0xDa1756Bb923Af5d1a05E277CB1E54f1D0A127890']
		))
	);

	const secondSpy = jest.spyOn(namicorn.ens, 'fetchAddress').mockImplementation(() =>
		new Promise((resolve, reject) => resolve('0xb0E7a465D255aE83eb7F8a50504F3867B945164C'))
	);
...
it('resolves .luxe name using ENS blockchain', async () => {
	...
	const spy = jest.spyOn(namicorn.ens, '_getResolutionInfo').mockImplementation(() =>
		new Promise((resolve, reject) => resolve(['0xf3dE750A73C11a6a2863761E930BF5fE979d5663', Number(0x00), '0xBD5F5ec7ed5f19b53726344540296C02584A5237']))
	);

	const secondSpy = jest.spyOn(namicorn.ens, 'fetchAddress').mockImplementation(() =>
		new Promise((resolve, reject) => resolve('0xf3dE750A73C11a6a2863761E930BF5fE979d5663'))
	);
...
it('resolves .luxe name using ENS blockchain null', async () => {
	...
	const spy = jest.spyOn(namicorn.ens, '_getResolutionInfo').mockImplementation(() =>
		new Promise((resolve, reject) => resolve(['0x0000000000000000000000000000000000000000', Number(0x00), '0x0000000000000000000000000000000000000000']))
	);
