...
import * as mockData from './testData/mockData.json';
import Ens from './ens';
...
const mockAPICalls = (topLevel: string, testName: string, url = MAINNET_URL) => {

	const mcdt = mockData as any;
	const tplvl = mcdt[topLevel] as any;
	const mockCall = tplvl[testName] as Array<any>;
...
it('provides empty response constant', async () => {
	...
	expect(response.meta.owner).toEqual(undefined);
});
