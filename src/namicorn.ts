...
class Namicorn {
  static readonly UNCLAIMED_DOMAIN_RESPONSE: unclaimed_domain = {
    addresses: {},
    meta: {
  ...
  async resolve(domain: string): Promise<NamicornResolveResponse> {
    if (this.blockchain) {
      return (await this.resolveUsingBlockchain(
        domain,
      )) as NamicornResolveResponse;
    } else {
  ...
  async resolveUsingBlockchain(
    domain: string,
  ): Promise<NamicornResolveResponse> {
    ...
    var result =
      method && ((await method.resolve(domain)) as NamicornResolveResponse);
