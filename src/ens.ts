...
export default class Ens {
	...
	_resolverCallToName(resolverContract: { methods: { name: (arg0: string) => { call: () => void; }; }; }, nodeHash: string) {
	...
	async _getResolutionInfo(nodeHash: string) {
	...
	async fetchAddress(resolver: string, nodeHash: string) {
	...
	async fetchPreviousOwner(domain: string) {
