...
export default class Ens {
	...
	_resolverCallToName(
		resolverContract: {
			methods: { name: (arg0: string) => { call: () => void } };
		},
		nodeHash: string,
	) {
	...
	_getResolver(nodeHash: string) {
		return this.ensContract.methods.resolver(nodeHash).call();
	}

	async _getResolutionInfo(nodeHash: string): Promise<[string, number, string]> {
	...
	async resolve(domain: string): Promise<EnsResolveResponse> {
		...
		return {
			...
			meta: {
				owner: owner === BLANK_ADDRESS ? null : owner,
				type: 'ens',
				ttl: Number(ttl),
			},
		};
	...
	async fetchAddress(resolver: string, nodeHash: string): Promise<string | null> {
	...
	async fetchPreviousOwner(domain: string): Promise<string | null> {
