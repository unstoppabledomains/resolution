...
export default class Ens {
    ...
    constructor(source: string | boolean = DEFAULT_SOURCE, options?: any) {
        if (source == true) {
    ...
    async reverse(address: string, currencyTicker: string) {
        ...
        const resolverAddress =
            this.options && this.options.isTest
                ? this.options.data[0]
                : await this.ensContract.methods.resolver(nodeHash).call();
    ...
    async resolve(domain) {
        ...
        if (this.options && this.options.isTest) {
            var [owner, ttl, resolver, mockAddress] = this.options.data;
        } else {
