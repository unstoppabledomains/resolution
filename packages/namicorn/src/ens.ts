...
export default class {
  ...
  constructor(source: string | boolean = DEFAULT_SOURCE) {
    ...
    this.deedContract = new this.web3.eth.Contract(
      deedInterface,
    )
    this.resolverContract = new this.web3.eth.Contract(
      resolverInterface,
    )
  ...
  async resolve(domain) {
    ...
    if (resolver == BLANK_ADDRESS) 
      resolver = null;
    if (owner == BLANK_ADDRESS) 
      owner = null;
  ...
  async fetchPreviousOwner(domain) {
    ...
    const [mode, deedAddress, registrationDateSeconds, value, highestBid] = await this.registrarContract.methods.entries(labelHash).call();
